...
Each week we would have a mandatory meeting in person, and additional online meetings to ensure work was getting done and to bring up problems team members might have been facing in order to solve them faster and not have one team member stuck on a bug for too long.

For the time between where the team would have meetings we created a WhatsApp group chat as a way to ask and answer quick questions we had on others' work or to make requests of other members that had arose.
...
This diagram illustrates the primary data flows between users, the backend application modules, the database layer and the notification infrastructure within our MediMate system.
...
The notification infrastructure utilises external services to deliver reliable messages to users. Firebase Cloud Message (FCM) delivers push notifications, while Gmail's SMTP server handles email notifications. Stored FCM tokens and notification preferences ensure dependable and consistent delivery.
...
The background scheduler triggers reminders at the precise time for each user using timezone-aware scheduling. It continuously evaluates medication schedules with minute-level precision to compute upcoming doses, detects overdue medications and triggers MediMate escalation, as required. The system also includes safeguards to prevent duplicate notifications and ensure reliable delivery.
...
### Mark Vaughan (123446326) - Backend Developer and Debugger<a id="mark_vaughan"></a>
...
The main section I worked on was the MediMates section. This section of the app handles all the features to do with the friend system that the app uses. This required a form to take the name of a user, check if they exist and send them a friend request. Friend relationships would be stored in a table with a reversed duplicate entry making it easy to gather friend data for use without gathering duplicates of relationships. Two sub-sections were made for the MediMate section, one showing the pending requests that a user had sent to other users and the other being incoming requests from other users.

I worked on the privacy feature for users accounts. I added a column in the user schema that stored if users allowed for the privacy settings in the profile page. I then worked across existing parts of the app to implement checks to enforce privacy if the user has chosen to share information with MediMates or not. I made modifications to the notification functions in order to make them adhere to the privacy settings. If they allow for info to be shared their MediMates will receive reminders if they have not taken their medication after an hour. Otherwise the schedule does not send any notification to the users MediMates.

Another feature optional feature dictated by the choice of privacy settings chosen was if friends could see your saved medications and symptoms logged. If information sharing is allowed in settings, if your friend goes into their MediMates page they will see a new link that would otherwise not appear, allowing them to see details regarding your recent symptoms you have logged such as the severity and notes and also information on your medications. This was done by making queries to both the `medications` table and the `symptoms` table through the flask route which would take the users user_id to join all these tables together to access the data. The feature allows for concerned friends or family to have a way to check any symptoms the user may have logged but forgot to inform them about providing a useful history.

I merged pages together that were originally separate. I took the originally separate profile and settings page and merged them into a single flask route that feels more connected and simple. The same was done with the friends page which originally gave all the features on the page their own route and HTML page. These were all put together to create a more complete friends page making it so the user would have to spend less time looking for the links to carry out different actions to do with their MediMates.

I spent a lot of time testing features across the entire app searching for any possible bugs that may be present in the code. I did this by testing many various entries in forms across the app to make sure that they worked as intended and then tested other features they worked with features that used the inputted data e.g. how entered medication worked wih the calendar features. I found the calendar originally set all future medication as missed so I added new status words and conditional statements in order to prevent users from being confused when viewing the month and week calendars. 

Overall my role focused on developing the friends system of the page which serves as a backbone to the idea and uniqueness of our app and searching for bugs across the entire app that may cause problems for users.
...
The main feature I worked on was the dashboard. It provides the user with visual insights of their medication progress. Here, I implemented a seven-day calendar view which shows whether doses were taken, missed, or not scheduled yet. This was the most challenging as designing this required calculating the current week and mapping each medication schedule to the current day. It had to also account for different medication frequency patterns which required additional logic. In the dashboard, I also implemented an adherence streak counter, a monthly adherence progress line chart which I had to use JavaScript for, dashboard statistics which includes the user's current adherence percentage and the number of active medications. This provides better insight into medication habits.
...
Overall, my role focused on making the core user-facing features and ensuring the frontend, backend, and database worked together effectively.

### Amina Baig - Frontend & UI Developer <a id="amina_baig"></a>
As the Frontend and UI Developer, my role involved designing the complete visual interface of MediMate. This involved using CSS, JavaScript and rewriting all existing HTML and Jinja templates to ensure they worked with the styling and layout of the application. Occasionally, I also updated app routed in app.py to work with any frontend features, such as colour-coding calenders. During our initial discussions on how to separate the work, I decided to volunteer to focus mainly on the visual design of the system as this allowed the interface to remain visually consistent, while allowing the other team members to dedicate time to developing the backend features. My work focused on improving overall usability, consistency, and creating the visual structure of the app so that the features could be easily understood by users.

At the beginning of the project, I created an initial prototype design to help visualise the layout and navigation of the application before development began. This allowed the team to understand how the main pages such as the dashboard, medication management pages and authentication screens would be structured. I mainly created hand-drawn wireframes and then improved those digitally using Canva.

I selected the theme colours of the app to be red and yellow. These colours were chosen because according to the research paper 'The Influence of Colour on Memory Performance' by Mariam Adawiah Dzulkifli and Muhammad Faiz Mustafar (referenced below), warm colours such as red and yellow have been shown to attract attention and support memory retention, which is relevant to MediMate where users are trying to keep track of their doses.
    
After the prototype stage, I implemented the interface using CSS, JavaScript and rewriting existing templates. Since our application targets users who may include elderly patients and sick people, usability and readability were important considerations. I designed the interface to be simple and clear, avoiding overly complex layouts. For multi-step processes like adding medication, I made the UI go one step at a time for ease of use.

I was responsible for styling all pages in the application. To ensure consistency across the application, I defined global CSS variables for color/size and some reusable styling patterns so that elements such as buttons, navigation links, and form inputs followed the same design language.
I also used JavaScript for to do some animations to make the interface more interactive such as for a sidebar and add medication steps.

I worked closely with the team to ensure that newly implemented features could be properly displayed in the user interface. This involved adapting layouts when new data structures were introduced and ensuring that forms correctly submitted data to the backend routes. 

To keep other aspects of our project consistent, I also designed our presentation and video pitch. 

Overall, my role focused on transforming the backend functionality into a cohesive, user-friendly and visually consistent interface. By designing the layout, implementing the styling system and ensuring that templates integrated properly with Flask and Jinja, I helped ensure that MediMate remained easy to navigate and accessible for its users.
...
As the project started progressing and the number of features increased, it sometimes became unclear which tasks should be prioritised and who should be responsible for completing them. After we completed the main development tasks, there were still some smaller features and fixes. Without a clear allocation, there was a risk that some tasks would've been forgotten or multiple people would have worked on the same task unnecessarily.
...
### Frontend Integration Challenges <a id="frontend_integration"></a>

One challenge during development was making sure the frontend templates worked properly with dynamic data from the Flask backend. Our pages rely on Jinja templates to display information such as medications, notifications and dashboard statistics. Due to this, when backend routes or database queries changed, the HTML templates often needed to be adjusted so the loops, conditionals and variables would render correctly without breaking the layout or styling. 

We communicated in our groupchat along with reading the commit messages to know when the templates needed updating.

Working through this helped us better understand how Flask passes data to templates and how Jinja is used to dynamically generate the user interface.
...
One important area is accessibility. Since the application is designed to support elderly users, additional features for accessibility such as a voice-over support, larger text mode or a dark/light theme toggle could have assisted users who may have visual impairments or difficulty navigating interfaces.
...
### Research
**The Influence of Colour on Memory Performance (Mariam Adawiah Dzulkifli, Muhammad Faiz Mustafar)** https://pmc.ncbi.nlm.nih.gov/articles/PMC3743993/